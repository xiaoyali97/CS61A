...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect is QueenAnt.queens[0]:
                return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        curr = self.place
        distance = 0
        while distance <= self.max_range and curr != hive:
            if distance >= self.min_range:
                bee_or_none = random_or_none(curr.bees)
                if bee_or_none:
                    return bee_or_none
            curr = curr.entrance
            distance += 1
...
class ScubaThrower(ThrowerAnt):
    """A ScubaThrower does not lose its armor when placed in Water."""
    name = 'Scuba'
    implemented = True
    food_cost = 6
    watersafe = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self is QueenAnt.queens[0]:
            self.throw_at(self.nearest_bee(colony.hive))
            behind = self.place.exit
            while behind:
                if behind.ant:
                    if behind.ant not in self.ants_behind:
                        behind.ant.damage = 2 * behind.ant.damage
                        self.ants_behind += [behind.ant]
                    if (behind.ant.container
                    and behind.ant.ant
                    and behind.ant.ant not in self.ants_behind):
                        behind.ant.ant.damage = 2 * behind.ant.ant.damage
                        self.ants_behind += [behind.ant.ant]
                behind = behind.exit
        else:
