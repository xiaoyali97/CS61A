...
class Keyboard:
    ...
    def __init__(self, *args):
        self.buttons = dict([(b.pos, b) for b in args])
...
def make_advanced_counter_maker():
    ...
    def counter_maker():
        ...
        curr_id = counter_id
        counter_id += 1
        def make_counter(message):
            nonlocal global_count
            if message == "count":
                counter[curr_id] += 1
                return counter[curr_id]
...
def make_to_string(front, mid, back, empty_repr):
    ...
    '()'
    """
    def to_string_func(lst):
        if lst is Link.empty:
            return empty_repr
        else:
            return front + str(lst.first) + mid + to_string_func(lst.rest) + back
...
def long_paths(tree, n):
    ...
    for b in tree.branches:
        for rest in long_paths(b, n-1):
            y.append(Link(tree.label, rest))
    return y
