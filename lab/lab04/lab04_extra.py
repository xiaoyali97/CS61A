...
def merge(lst1, lst2):
    ...
    lst = lst1 + lst2
    sort_lst = [min(lst)]
    while len(sort_lst) < len(lst):
        sort_lst += [min([x for x in lst if x not in sort_lst])]
    return sort_lst
...
def put_piece(board, max_rows, column, player):
    ...
    for r in range(1,max_rows):
        if get_piece(board, r, column) != '-':
            return (r-1, replace_elem(board,r-1,replace_elem(board[r-1],column,player)))
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    h_win = check_win_row(board, max_rows, max_cols, num_connect, row, player)
    v_win = check_win_column(board, max_rows, max_cols, num_connect, col, player)
    return diagonal_win or h_win or v_win
###############################################################
