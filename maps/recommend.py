...
def find_centroid(cluster):
    ...
    lon = mean([restaurant_location(r)[1] for r in cluster])
    return [lat,lon]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, r_squared = s_xy / s_xx, s_xy ** 2 / (s_xx * s_yy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    a = mean(ys) - b * mean(xs)
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    lst_predictor = [find_predictor(user, reviewed, f) for f in feature_fns]
    return max(lst_predictor, key = lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            rating_dict.update({name: user_rating(user, name)})
        else:
            rating_dict.update({name: predictor(r)})
    return rating_dict
    # END Question 9
