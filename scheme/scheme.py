...
def eval_all(expressions, env):
    ...
    while curr is not nil:
        if curr.second is nil:
            val = scheme_eval(curr.first, env, True) #Tail context
        else:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('number of argument values does not match with \
            the number of formal parameters')
        else:
            curr_formal, curr_val = formals, vals
            while curr_formal is not nil:
                child.define(curr_formal.first, curr_val.first)
                curr_formal, curr_val = curr_formal.second, curr_val.second
        # END PROBLEM 11
            return child
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambda_exprs = Pair(target.second, expressions.second)
...
def do_and_form(expressions, env):
    ...
    if expressions.second is nil:
        result = scheme_eval(expressions.first, env, True) #Tail context
    else:
...
def make_let_frame(bindings, env):
    ...
    symbols = bindings.map(lambda bind_list: bind_list.first)
    check_formals(symbols)
    values = bindings.map(lambda bind_list: scheme_eval(bind_list.second.first, env))
