...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    quotient, remainder = score // 10, score % 10
    return abs(quotient - remainder) + 2
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            score_this_turn = take_turn(strategy0(score0,score1), score1, dice)
            score0 += score_this_turn
        else:
            score_this_turn = take_turn(strategy1(score1,score0), score0, dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def announce(score0, score1):
        ...
        if gain > previous_high:
            ...
            return announce_highest(who, gain, curr)
        else:
            return announce_highest(who, previous_high, curr)
    return announce
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score, min_roll = 0, 1
    average_score = make_averaged(roll_dice, num_samples)
    for i in range(1, 11):
        result = average_score(i, dice)
        if result > max_score:
            max_score = result
            min_roll = i
    return min_roll
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if score + bacon_score < opponent_score \
       and is_swap(score + bacon_score, opponent_score):
        return 0
    elif bacon_score >= margin and \
        not is_swap(score + bacon_score, opponent_score):
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    First try to force a swap, then stay at swap_strategy with num_rolls = 5.
    """
    # BEGIN PROBLEM 12
    if score + 1 < opponent_score and is_swap(score + 1, opponent_score):
        return 10
    else:
